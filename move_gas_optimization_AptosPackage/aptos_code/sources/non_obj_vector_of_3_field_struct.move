module move_gas_optimization::non_obj_vector_of_3_field_struct{
    use std::vector;
    ...
    struct Vector_of_Struct has store, drop{
        vec: vector<Three_Slot_Struct>
    }

    /// Creates & pushes elements to vector of 3-field struct
    public entry fun non_obj_create_pushback_vector_of_struct(){
        //1) Create empty vector to store structs
        let temp_vector = vector::empty<Three_Slot_Struct>();
